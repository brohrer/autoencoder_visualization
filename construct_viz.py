"""
> python3 construct_viz.py

Generates and saves a sequence of neural network visualizations,
showing each step of construction.
"""
import matplotlib
matplotlib.use("agg")
...
N_NODES_BY_LAYER = [18, 13, 10, 5, 8, 11, 18]
...
def create_background(p):
    """
    Create the figure and fill it corner to corner with a single Axes instance.
    This turns the whole figure into a big blank canvas that we can draw on.
    """
    fig = plt.figure(
        edgecolor=TAN,
        facecolor=GREEN,
...
def add_input_image(fig, image_axes, p, filler_image):
    ...
    add_filler_image(
       ...
       p["input"]["n_rows"],
       p["input"]["n_cols"],
       filler_image=filler_image)
    image_axes.append([ax_input])


def add_node_images(fig, image_axes, i_layer, p, filler_image):
    """
...
def add_filler_image(ax, n_im_rows, n_im_cols, filler_image=None):
    """
    Add a chunk of image as a placeholder.
    """
    if filler_image is None:
...
def add_layer_connections(ax_boss, image_axes):
    ...
    for i_start_layer in range(len(image_axes) - 1):
        ...
        for ax_start in image_axes[i_start_layer]:
            ...
            y_starts += get_evenly_spaced_points(
                y_start_min, y_start_max, n_end_nodes)
        ...
        for ax_end in image_axes[i_start_layer + 1]:
            ...
            y_ends += get_evenly_spaced_points(
                y_end_min, y_end_max, n_start_nodes)

        y_ends_reordered = (
            list(
                np.reshape(
                    np.array(y_ends),
                    (n_end_nodes, n_start_nodes)
                ).transpose().ravel()
            )
        )

        for y_start, y_end in zip(y_starts, y_ends_reordered):
            plot_connection(ax_boss, x_start, x_end, y_start, y_end, alpha=1)
...
def get_evenly_spaced_points(start, end, n_pts):
    """
    Spacing between points will be d.
    Spacing on either end will be d plus a fraction.
    """
    end_spacing = 0.5
    delta = (end - start) / (n_pts - 1 + end_spacing * 2)
    evenly_spaced = np.linspace(
        start + end_spacing * delta,
        end - end_spacing * delta,
        num=n_pts)
    return list(evenly_spaced)
